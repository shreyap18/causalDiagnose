...
#' @param fill_rest_subsamp A logical value on whether to fill the rest of the subsamples with the converged rate (e.g 1) when early stopping is used, default is True
#'
...
#' name_files, name_out, run_test = FALSE)

run_procedure <- function(data, dir, sampsizes, names_col_cddr, name_files, name_out, fit_resid_yonx = function(x) {NA}, predict_yonx = function(x) {NA}, fit_resid_xony = function(x) {NA}, predict_xony = function(x) {NA}, run_parallel = T, run_causal_method = function(x) {NA}, run_lingam = T, run_test = T, clear_files = T, nsubsamp = 100, nboot = 100, alpha = 0.05, early_stop = T, early_stop_thresh = 2, fill_rest_subsamp = T){
  early_stop_iter = 0
  ...
  for (s in 1:length(sampsizes)){
    ...
    cddr_samp <- run_procedure_samp(data, dir, sampsize, fit_resid_yonx, predict_yonx, fit_resid_xony, predict_xony, run_parallel, run_causal_method, run_lingam, run_test, clear_files, nsubsamp, nboot, alpha)
